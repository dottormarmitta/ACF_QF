...
import com.dottormarmitta.acf.products.ACFExamProductWithCI;
...
public class ProductValuation {
	...
	public static void main(String[] args) {
		System.out.println("This interface will price our product.");
		System.out.println();
		...
		double today = 0.0;
		double maturity = 0.0;
		System.out.println("For high maturity, please use our parallel implementation!");
		System.out.print("What is maturity of the bonds? ");
		maturity = s.nextDouble();
		System.out.println();
		int dt = (int) (maturity*12);
		...
		System.out.println();
		double d = 0.0;
		System.out.print("Enter the default threshold: ");
		d = s.nextDouble();
		System.out.println();
		double notional = 0.0;
		System.out.print("Enter the initial notional: ");
		notional = s.nextDouble();
		System.out.println();
		int ns = 0;
		System.out.println("For high number of simulation, please use our parallel implementation!");
		System.out.println("Here number of simulations should be between 100k - 500k");
		System.out.print("Enter number of simulation you want to perform: ");
		ns = s.nextInt();
		System.out.println();
		...
		// Product
		double[] fast;
		FinancialProduct fastProduct = new ACFExamProductWithCI(rf, vols, brownian, zcbc, d, tenor, ns);
		fast = fastProduct.getValue();
		
		double juniorExp = fast[0];
		double juniorVar = Math.sqrt(fast[1]/ns);
		
		double mezzanExp = fast[2];
		double mezzanVar = Math.sqrt(fast[3]/ns);
		
		double seniorExp = fast[4];
		double seniorVar = Math.sqrt(fast[5]/ns);
		
		System.out.println("Junior 95% confidence interval is:    " + (notional*juniorExp - 3*juniorVar) + " < JuniorPrice < " + (notional*juniorExp + 3*juniorVar));
		System.out.println("Junior tranche expected value is: " + notional*juniorExp);
		System.out.println();
		System.out.println("Mezzan 95% confidence interval is:    " + (notional*mezzanExp - 3*mezzanVar) + " < MezzanPrice < " + (notional*mezzanExp + 3*mezzanVar));
		System.out.println("Mezzan tranche expected value is: " + notional*mezzanExp);
		System.out.println();
		System.out.println("Senior 95% confidence interval is:    " + (notional*seniorExp - 3*seniorVar) + " < SeniorPrice < " + (notional*seniorExp + 3*seniorVar));
		System.out.println("Senior tranche expected value is: " + notional*seniorExp);
	}
}
